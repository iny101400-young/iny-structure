...
★ 짚힌 자리는 재료를 빼는 게 아니라 값을 가린다.
위키에서만 빼면 값이 재료에 그대로 남고, 재료는 06 도 07 도 읽는다.
재료 md 본문의 값을 [가림] 으로 바꾸고 머리말에 sensitive · masked_at 을 붙인다.
원본 파일은 안 건드린다. 그러면 이 검사가 다음 번에 안 걸린다.
...
설명 = {
    ...
    "I_개인정보": "개인정보가 아직 안 가려진 자리입니다. 빼지 말고 값을 가리세요",
}
